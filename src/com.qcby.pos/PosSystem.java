...
public class PosSystem {
    ...
    public PosSystem(int goodsTotal, int googsIndex) {
        ...
        this.dataFilePath = "goods_data.json";
        this.recordsFilePath = "purchase_records.json";
    ...
    public Boolean addGoods(Goods goods){
        for (Goods allGood : allGoods) {
            if (allGood.getId().equals(goods.getId())) {
                System.out.println("商品id已存在,新增失败");
                return false;
            }
        }
    ...
    public boolean updateGoods(Integer id, Goods updatedGoods) {
        for (int i = 0; i < this.googsIndex; i++) {
            Goods goods = this.allGoods.get(i);
            if (goods.getId().equals(id)) {
                // 检查新的商品ID是否与其他商品冲突（除了当前商品）
                if (!updatedGoods.getId().equals(id)) {
                    for (int j = 0; j < this.googsIndex; j++) {
                        if (j != i && this.allGoods.get(j).getId().equals(updatedGoods.getId())) {
                            System.out.println("商品id已存在,修改失败");
                            return false;
                        }
                    }
                }
                ...
                // 直接更新商品信息
                goods.setId(updatedGoods.getId());
